...
// BXD-46 — a sub-task (one with a parent_id) renders dotted: the root task's
// number, then each level's 1-based position among its parent's children by
// creation order (num as the tiebreak). CGB-3 under CGB-2 -> CGB-2.1. `num`
// stays authoritative; this is derived, so deleting an earlier sibling shifts
// the ones after it — `id` and `<prefix>-<num>` are the stable refs.
// Kept byte-for-byte in step with the dashboard's src/lib/data/task-key.ts.
function siblingsOf(parentId, tasks) {
  return tasks
...
export function taskDisplayKey(task, tasks, prefix) {
  ...
  const root = chain[0];
  if (root.num == null) return `${prefix}-${task.num}`;
  let key = `${prefix}-${root.num}`;
  for (let depth = 1; depth < chain.length; depth += 1) {
    const parent = chain[depth - 1];
    const ordinal = siblingsOf(parent.id, tasks).findIndex((t) => t.id === chain[depth].id) + 1;
    key += `.${ordinal || 1}`;
  }
  return key;
}

// Resolve a spoken ref to a task: "CGB-2.1" walks the child ordinals; "CGB-19"
// or "19" matches `num` directly (still works for a task that now displays
// dotted). Throws on a wrong prefix; returns undefined if nothing matches.
export function resolveTaskRef(ref, tasks, prefix) {
  const upper = String(ref).trim().toUpperCase();
  const dotted = /^(?:([A-Z]+)-)?(\d+(?:\.\d+)+)$/.exec(upper);
  if (dotted) {
    if (dotted[1] && dotted[1] !== prefix) throw new Error(`task ${ref}: prefix ${dotted[1]} is not ${prefix}`);
    const parts = dotted[2].split(".").map(Number);
    let cur = tasks.find((t) => t.num === parts[0] && !t.parent_id);
    for (let i = 1; cur && i < parts.length; i += 1) {
      cur = siblingsOf(cur.id, tasks)[parts[i] - 1];
    }
    return cur;
  }
  const keyed = /^([A-Z]+)-(\d+)$/.exec(upper);
  if (keyed) {
    if (keyed[1] !== prefix) throw new Error(`task ${ref}: prefix ${keyed[1]} is not ${prefix}`);
    return tasks.find((t) => t.num === Number(keyed[2]));
  }
  if (/^\d+$/.test(upper)) return tasks.find((t) => t.num === Number(upper));
  return undefined;
}
...
export async function setTaskStatus(dir, slug, ref, target, now) {
  ...
  if (!task) throw new Error(`unrecognised or unknown task ref: ${ref} (try ${prefix}-13, 13, or ${prefix}-2.1)`);
...
export function renderBoardBody(tasks, prefix, stamp) {
  ...
  const key = (task) => taskDisplayKey(task, tasks, prefix) ?? `${prefix}-${task.num}`;
  const line = (task) => `- ${key(task)} — ${clipTitle(task.title)}`;
